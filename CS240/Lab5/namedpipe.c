...
/* Check for existence of named pipe and create it if it doesn't exist */
	...
	if(access("my_pipe", F_OK) == -1) /*doesn’t exist */
		if(mknod("my_pipe", 010600, 0) == 0)	printf("Named Pipe created successfully \n");
		else {
...
the pipe accordingly */
	if (argc == 2) {
		if (strcmp (argv[1], "reader")==0)	namedpipe = open("my_pipe", O_RDONLY);
		else if (strcmp(argv[1], "writer")==0)	namedpipe = open("my_pipe", O_WRONLY);
		else	paramerror = true;
	}
	else	paramerror = true;

	if (paramerror)	printf("Incorrect usage: Use namedpipe <reader | writer>\n");
	else if (namedpipe<0) printf("Couldn't open named pipe\n");
...
/* Now do reader or writer actions next */
	...
	/*Writer outputs 10 messages to pipe */
		...
		/* convert i to string as part of message */
			...
			printf("writer: Sent message <%s> to namedpipe\n",message);
		}
	else 
		for(i=1; i<=10; i++) {
